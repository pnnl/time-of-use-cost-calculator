...
class TestUnitConversion(unittest.TestCase):
    ...
    def test_extract_unit_from_column_name(self):
        ...
        self.assertEqual(
            self.calculator._extract_unit_from_column_name('Electricity:Facility [W](Hourly)'),
            'W'
        )
        self.assertEqual(
            self.calculator._extract_unit_from_column_name('Demand [kW](Monthly)'),
            'kW'
        )
        # Test no unit
        self.assertIsNone(
    ...
    def test_convert_power_to_kw(self):
        ...
        self.assertAlmostEqual(
            self.calculator._convert_power_to_kw(1000, 'W'),
            1.0
        )
        # kW to kW (no conversion)
        self.assertAlmostEqual(
            self.calculator._convert_power_to_kw(5.0, 'kW'),
            5.0
        )
        # MW to kW
        self.assertAlmostEqual(
            self.calculator._convert_power_to_kw(0.001, 'MW'),
            1.0
        )
        # Case insensitive
        self.assertAlmostEqual(
...
class TestRateLoading(unittest.TestCase):
    ...
    def test_load_rate_missing_file(self):
        ...
        calculator = EnergyCostCalculator(
            rate_json_path='/nonexistent/path/rate.json',
            data=None
        )
