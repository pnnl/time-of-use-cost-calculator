...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'geb_script'))
...
class CalculatorComparison:
    ...
    def run_energy_cost_calculator(self):
        ...
        try:
            ...
            energy_cost = calculator.data['energy_charge'].sum() if 'energy_charge' in calculator.data else 0
            demand_cost = calculator.data['demand_charge'].sum() if 'demand_charge' in calculator.data else 0
    ...
    def run_tou_calculator(self, save_csv=True):
        ...
        try:
            ...
            rate_name = os.path.basename(self.rate_json_path).replace('_in_openei_query_format.json', '')
            ...
            # Calculate totals
            energy_cost = sum([v for k, v in summary.items() if 'charge_energy_' in k and 'annual' in k])
            ...
            demand_cost = sum([v for k, v in summary.items() 
                             if 'charge_peak' in k 
                             and 'annual' in k 
                             and 'total_annual' not in k])  # Exclude total to avoid double-counting
            
            demand_flat_cost = sum([v for k, v in summary.items() if 'charge_demandflat_' in k and 'annual' in k])
    ...
    def compare_results(self):
        ...
        pct_diff = (diff / calc1['energy_cost'] * 100) if calc1['energy_cost'] != 0 else 0
        print(f"Difference:                 ${diff:>15,.2f} ({pct_diff:+.2f}%)")
        ...
        pct_diff = (diff / calc1['demand_cost'] * 100) if calc1['demand_cost'] != 0 else 0
        print(f"Difference:                 ${diff:>15,.2f} ({pct_diff:+.2f}%)")
        ...
        print(f"tou_calculator.py:          ${calc2['fixed_cost']:>15,.2f} (not calculated)")
...
if __name__ == "__main__":
    ...
    for csv_path in csv_files:
        ...
        for rate_path in rate_files:
            rate_name = os.path.basename(rate_path).replace('_in_openei_query_format.json', '')
    ...
    print("\nSUMMARY:")
    print("-"*80)
    success_count = sum(1 for r in all_results.values() 
    ...
    for test_name, result in all_results.items():
        ...
        if calc1.get('success') and calc2.get('success'):
            ...
            if total_diff >= tolerance or energy_diff >= tolerance or demand_diff >= tolerance:
    ...
    print(f"✓ All calculators produce identical results (within ${tolerance:.2f} tolerance)")
    sys.exit(0)
