...
class EnergyCostCalculator:
    ...
    def __init__(self, rate_label = None, rate_json_path = None, data = None, include_demand_cost = False, include_energy_cost = False, include_fixed_cost = False, number_of_meters = 1, electricity_demand_var_name = None, add_adjustment_to_rate = True):
        ...
        if number_of_meters < 1:
            logging.warning(f"Number of meters cannot be less than 1. Setting number_of_meters to 1.")
            number_of_meters = 1
    ...
    def _load_rate(self):
        ...
        if self.rate_json_path is not None:
            ...
            try:
                ...
                logging.error(f"Error fetching rate from API with label {self.rate_label}: {e}")
                return None
    ...
    def _convert_power_to_kw(self, value, from_unit):
        ...
        if from_unit_lower == 'w':
            ...
            logging.warning(f"Unknown power unit '{from_unit}'. Assuming it's already in kW.")
            return value
    ...
    def _validate_demand_units(self, data_unit):
        ...
        if expected_unit:
            ...
            if expected_unit.lower() != 'kw':
                logging.warning(f"Rate expects demand in '{expected_unit}' but calculator converts to 'kW'. Results may be incorrect.")
                return False
        else:
            logging.info(f"Rate does not specify demand units. Data unit: {data_unit}, converting to kW")
    ...
    def _validate_energy_units(self, data_unit):
        ...
        if expected_unit:
            ...
            logging.info(f"Data provides power in: {data_unit}, will be converted to kWh")
            
            # Verify conversion produces the expected unit
            if expected_unit.lower() != 'kwh':
                logging.warning(f"Rate expects energy in '{expected_unit}' but calculator converts to 'kWh'. Results may be incorrect.")
                return False
        else:
            logging.info(f"Rate does not specify energy units. Data unit: {data_unit}, converting to kWh")
    ...
    def get_total_cost(self, add_adjustment_to_rate = True):
        ...
        energy_cost = self.calculate_energy_cost(add_adjustment_to_rate) if self.include_energy_cost else 0

        # Fixed charges calculation
        fixed_cost = self.calculate_fixed_charge_cost() if self.include_fixed_cost else 0

        # Demand cost calculation
        demand_cost = self.calculate_demand_cost(add_adjustment_to_rate) if self.include_demand_cost else 0
    ...
    def calculate_demand_cost(self, add_adjustment_to_rate = True):
        ...
        if self.electricity_demand_var_name is None:
            logging.error("Electricity demand variable name must be provided for demand cost calculation.")
            return 0
        
        # Get variable names
        day_type_var_cols = [col for col in self.data.columns if 'site day type index' in col.lower()]
        if not day_type_var_cols:
            logging.error("No 'site day type index' column found in data.")
            return 0
        day_type_var_name = day_type_var_cols[0]
        
        demand_var_cols = [col for col in self.data.columns if col.lower() == self.electricity_demand_var_name.lower()]
        if not demand_var_cols:
            logging.error(f"Electricity demand variable '{self.electricity_demand_var_name}' not found in data columns.")
            return 0
        ...
        if demand_unit is None:
            logging.warning(f"Could not extract demand unit from variable name '{demand_var_name}'. Assuming demand is in watts (W).")
            demand_unit = 'W'
        ...
        if "demand" in rate:
            ...
            logging.error(f"Demand rate data not found in rate '{self.rate_label}'. Demand cost calculation will be set to 0.")
            return 0
        ...
        if "flatdemandstructure" in rate_demand:
            ...
            monthly_max_demand_indexes = self.data.groupby(self.data.index.month)["demand_usage_kW"].idxmax()

            # Get flat demand charge for each month
            for month_num, monthly_max_demand_index in monthly_max_demand_indexes.items():
                rate_demand_period = rate_demand["flatdemandmonths"].loc[month_num]["flat"]
                ...
                for _, tier_row in rate_demand_structure_df.iterrows():
                    max_kW = tier_row.get("max", float('inf')) if "max" in tier_row.index else float('inf')
                    if self.data.at[monthly_max_demand_index, "demand_usage_kW"] > max_kW:
                        continue
                    else:
                ...
                if not tier_found:
                    ...
                    logging.warning(f"Demand usage exceeds all tier maximums for month {monthly_max_demand_index.month}. Using highest tier.")               
                ...
                charge_rate = period_tier_rate["rate"].values[0] if len(period_tier_rate) > 0 else 0
                if "adj" in period_tier_rate.columns and len(period_tier_rate) > 0 and add_adjustment_to_rate:
                ...
                self.data.at[monthly_max_demand_index, "demand_charge_rate"] = charge_rate
                # Store flat demand charge separately and add to total
                flat_charge = charge_rate * self.data.at[monthly_max_demand_index, "demand_usage_kW"]
                self.data.at[monthly_max_demand_index, "demand_charge_flat"] = flat_charge
        ...
        if "demandratestructure" in rate_demand:
            ...
            for idx, row in self.data.iterrows():
                ...
                try:
                    ...
                    logging.warning(f"No rate schedule found for month {idx.month}, hour {idx.hour}. Skipping this timestep.")
                    continue
            ...
            for month_num, month_data in self.data.groupby(self.data.index.month):
                for period in month_data["demand_peak_period"].unique():
                    ...
                    if len(period_data) > 0:
                        ...
                        max_demand_kW = period_data.loc[max_demand_idx, "demand_usage_kW"]
                        charge_rate = period_data.loc[max_demand_idx, "demand_charge_rate"]
                        ...
                        if charge_rate > 0:
                            ...
                            self.data.at[max_demand_idx, "demand_charge_tou"] += tou_charge
    ...
    def calculate_fixed_charge_cost(self):
        ...
        if 'fixedchargeunits' in self.rate:
            ...
            if units == '$/day':
                ...
                fixed_charge_cost = days * (self.rate.get('fixedchargefirstmeter', 0) + self.rate.get('fixedchargeeaaddlmeter', 0) * (self.number_of_meters - 1))
            elif units == '$/month':
                months = self.data.index.month.nunique()
                fixed_charge_cost = months * (self.rate.get('fixedchargefirstmeter', 0) + self.rate.get('fixedchargeeaaddlmeter', 0) * (self.number_of_meters - 1))
            elif units == '$/year':
                years = self.data.index.year.nunique()
                fixed_charge_cost = years * (self.rate.get('fixedchargefirstmeter', 0) + self.rate.get('fixedchargeeaaddlmeter', 0) * (self.number_of_meters - 1))
            else:
                logging.warning(f"Unrecognized fixed charge units '{units}' for rate '{self.rate_label}'. Fixed charge cost will be set to 0.")
    ...
    def calculate_energy_cost(self, add_adjustment_to_rate = True):
        ...
        """
        if self.electricity_demand_var_name is None:
            logging.error("Electricity demand variable name must be provided for energy cost calculation.")
            return 0
        ...
        if "energy" in rate:
            ...
            logging.error(f"Energy rate data not found in rate '{self.rate_label}'. Energy cost calculation will be set to 0.")
            return 0
        ...
        for idx, row in self.data.iterrows():
            ...
            try:
                ...
                logging.warning(f"No rate schedule found for month {idx.month}, hour {idx.hour}. Using tier 0.")
                selected_tier = 0
                charge_rate = 0
            ...
            for j, tier_row in rate_energy_structure_df.iterrows():
                max_kWh = tier_row.get("max", float('inf')) if "max" in tier_row else float('inf')
                if tier_row["unit"] != "kWh":
                    logging.warning(f"Tier {tier_row['tier']} uses {tier_row['unit']} for rate '{self.rate_label}'. Only 'kWh' is supported for energy rates. This tier will be skipped in energy cost calculation.")
                    continue
            ...
            if not tier_found and len(rate_energy_structure_df) > 0:
                # Use the last (highest) tier if usage exceeds all tier maxes
                valid_tiers = rate_energy_structure_df[rate_energy_structure_df["unit"] == "kWh"]
            ...
            if "rate" in rate_energy_structure_df.columns:
                ...
                logging.warning(f"The 'rate' column not found in energy rate structure for rate '{self.rate_label}'. Energy cost for this timestep will be set to 0.")
                charge_rate = 0
            ...
            if idx == self.data.index[0]:
                ...
                if len(self.data.index) > 1:
                    time_diff_hours = (self.data.index[1] - self.data.index[0]).total_seconds() / 3600
                else:
    ...
    def get_summary(self):
        ...
        energy_cost = self.data['energy_charge'].sum() if 'energy_charge' in self.data.columns else 0
        demand_cost = self.data['demand_charge'].sum() if 'demand_charge' in self.data.columns else 0
        demand_flat_cost = self.data['demand_charge_flat'].sum() if 'demand_charge_flat' in self.data.columns else 0
        demand_tou_cost = self.data['demand_charge_tou'].sum() if 'demand_charge_tou' in self.data.columns else 0
        fixed_cost = self.calculate_fixed_charge_cost() if self.include_fixed_cost else 0
        ...
        if 'energy_charge' in self.data.columns:
            monthly_energy = self.data.groupby(self.data.index.month)['energy_charge'].sum()
            for month in range(1, 13):
                summary[f'energy_cost_month_{month:02d}'] = monthly_energy.get(month, 0)
        
        if 'energy_usage_kWh' in self.data.columns:
            monthly_kwh = self.data.groupby(self.data.index.month)['energy_usage_kWh'].sum()
            for month in range(1, 13):
                summary[f'energy_kwh_month_{month:02d}'] = monthly_kwh.get(month, 0)
        
        if 'demand_charge' in self.data.columns:
            monthly_demand_cost = self.data.groupby(self.data.index.month)['demand_charge'].sum()
            for month in range(1, 13):
                summary[f'demand_cost_month_{month:02d}'] = monthly_demand_cost.get(month, 0)
        
        if 'demand_usage_kW' in self.data.columns:
            monthly_peak_kw = self.data.groupby(self.data.index.month)['demand_usage_kW'].max()
            for month in range(1, 13):
                summary[f'peak_kw_month_{month:02d}'] = monthly_peak_kw.get(month, 0)
        
        # Add fixed charges by month
        if self.include_fixed_cost:
            monthly_fixed = fixed_cost / self.data.index.month.nunique() if self.data.index.month.nunique() > 0 else 0
            for month in range(1, 13):
    ...
    def export_detailed_results(self, output_folder='outputs', case_name='case'):
        ...
        timestep_cols = [col for col in self.data.columns if any(x in col.lower() for x in 
                        ['energy_charge', 'energy_usage', 'energy_charge_rate', 'fraction_of_hour',
                         'demand_charge', 'demand_usage', 'demand_charge_rate', 'demand_peak_period'])]
        
        if timestep_cols:
            timestep_file = os.path.join(output_folder, f'{case_name}_timestep_charges.csv')
        ...
        if 'energy_charge' in self.data.columns:
            ...
            monthly_file = os.path.join(output_folder, f'{case_name}_energy_monthly.csv')
        ...
        if 'energy_charge' in self.data.columns:
            energy_daily = self.data.groupby([self.data.index.month, self.data.index.day]).agg({
                'energy_charge': 'sum',
                'energy_usage_kWh': 'sum'
            })
        ...
        if 'demand_charge' in self.data.columns:
            ...
            demand_monthly_file = os.path.join(output_folder, f'{case_name}_demand_monthly.csv')
