...
class DateTimeEP:
    ...
    def transform(self) -> pd.DataFrame:
        ...
        self.df = self.df.shift(periods=-1, freq="s")
    ...
    def do_not_apply_holidays(self, day_type_var_name: str) -> pd.DataFrame:
        ...
        for day_idx, day in self.df.groupby(self.df.index.date):
            ...
            if prev_day == 7:  # previous day is Saturday
                ...
                logging.error(
                    f"Two consecutive days are not normal days at index {day_idx}, unexpected behavior"
                )
                return self.df
            newdaytypes.extend([cur_day] * len(day))
            prev_day = cur_day
        ...
        if len(newdaytypes) != len(self.df):
            logging.error(
                f"New day types length ({len(newdaytypes)}) does not match DataFrame length ({len(self.df)})"
            )
            return self.df
