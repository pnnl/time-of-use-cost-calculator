...
def get_by_label(label: str, apikey: str, export_to_file: bool = False, output_path: str = None):
    ...
    if "items" not in data:
        logger.warning(f"Something is wrong with {label}, query return does not contain 'items'")
        print(data)
        return None
    if len(data["items"]) != 1:
        logger.warning(f"Something is wrong with {label}, query return is not a unique item")
        return None
    ...
    if export_to_file:
        ...
        if output_path is None:
            ...
            safe_name = "".join(c if c.isalnum() or c in ('-', '_') else '_' for c in rate_name)
...
def get_unique_fixedchargeunits(apikey: str):
    ...
    for i, label in enumerate(all_labels):
        try:
            ...
            if (i + 1) % 1000 == 0:
                logger.info(f"Processed {i + 1}/{total} rates... Found {len(unique_units)} unique units so far")
    ...
    logger.info(f"Completed! Processed {total} rates and found {len(unique_units)} unique fixedchargeunits values")
