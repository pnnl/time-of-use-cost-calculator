...
class DataForCostCalculation:
    def __init__(self, path_to_data_file, data_source='EnergyPlus', year=2000, use_holidays=False):
    ...
    def preprocess_data(self, year):
        ...
        if self.data_source.lower() == 'energyplus':
            ...
            dst_type_var_names = [col for col in self.data.columns if 'site daylight saving time status' in col.lower()]
            day_type_var_names = [col for col in self.data.columns if 'site day type index' in col.lower()]
            if len(dst_type_var_names) > 0 and len(day_type_var_names) > 0:
                self.data = date_helper.add_dst_clocktime(day_type_col=day_type_var_names[0], dst_type_col=dst_type_var_names[0])
